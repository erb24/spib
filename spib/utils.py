...
def prepare_data(data_list, label_list, weight_list, output_dim, train_indices,
	...
	r"""This can be used to prepare the data for spib training and validation.
		...
		Ut_list: List of trajectories of the potential energy
			Values of the potential energy used to calculate the energy profiles in the latent space
	"""
	...
	# both weights and Ut are optional, depending on the trajectory and training method
	if weight_list is None:
		if Ut_list is None:
			train_dataset = TimeLaggedDataset(train_data, train_labels, None, None, lagtime=lagtime,
											  subsampling_timestep=subsampling_timestep,
											  output_dim=output_dim, device=device)
			test_dataset = TimeLaggedDataset(test_data, test_labels, None, None, lagtime=lagtime,
											 subsampling_timestep=subsampling_timestep,
											 output_dim=output_dim, device=device)

		else:
			train_Ut = [Ut_list[i] for i in train_indices]
			test_Ut = [Ut_list[i] for i in test_indices]

			train_dataset = TimeLaggedDataset(train_data, train_labels, None, train_Ut, lagtime=lagtime,
											  subsampling_timestep=subsampling_timestep,
											  output_dim=output_dim, device=device)
			test_dataset = TimeLaggedDataset(test_data, test_labels, None, test_Ut, lagtime=lagtime,
											 subsampling_timestep=subsampling_timestep,
											 output_dim=output_dim, device=device)
	else:
		...
		if Ut_list is None:
			train_dataset = TimeLaggedDataset(train_data, train_labels, train_weights, None, lagtime=lagtime,
											  subsampling_timestep=subsampling_timestep,
											  output_dim=output_dim, device=device)
			test_dataset = TimeLaggedDataset(test_data, test_labels, test_weights, None, lagtime=lagtime,
											 subsampling_timestep=subsampling_timestep,
											 output_dim=output_dim, device=device)
			...
			train_dataset = TimeLaggedDataset(train_data, train_labels, train_weights, train_Ut, lagtime=lagtime,
											  subsampling_timestep=subsampling_timestep,
											  output_dim=output_dim, device=device)
			test_dataset = TimeLaggedDataset(test_data, test_labels, test_weights, test_Ut, lagtime=lagtime,
											 subsampling_timestep=subsampling_timestep,
											 output_dim=output_dim, device=device)
...
class TimeLaggedDataset(torch.utils.data.Dataset):
	...
	def __init__(self, data_list, label_list, weight_list=None, Ut_list=None, lagtime=1, subsampling_timestep=1,
		...
		self.data_Ut = torch.from_numpy(np.concatenate([data_init_list[i][4] for i in range(len(data_init_list))],
															axis=0)).float().to(device)
...
def data_init(dt, timestep, traj_data, traj_label, traj_weights, traj_Ut):
	...
	if traj_Ut is not None:
		assert len(traj_data)==len(traj_Ut)
		Ut = traj_Ut[:(len(traj_data)-dt):timestep]
	else:
		Ut = None
